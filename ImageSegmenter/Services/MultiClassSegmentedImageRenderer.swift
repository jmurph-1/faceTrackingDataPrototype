...
class MultiClassSegmentedImageRenderer {
  ...
  private func makeTextureFromCVPixelBuffer(pixelBuffer: CVPixelBuffer, textureFormat: MTLPixelFormat) -> MTLTexture? {
    ...
    let result = CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault, textureCache, pixelBuffer, nil, textureFormat, width, height, 0, &cvTextureOut)
  ...
  private func allocateOutputBufferPool(with formatDescription: CMFormatDescription,
                                       outputRetainedBufferCountHint: Int,
                                       needChangeWidthHeight: Bool = false) -> (
    ...
    let poolCreateStatus = CVPixelBufferPoolCreate(kCFAllocatorDefault,
                                                  nil,
    ...
    let pixelBufferCreateStatus = CVPixelBufferPoolCreatePixelBuffer(kCFAllocatorDefault, pixelBufferPool!, &pixelBuffer)
    if pixelBufferCreateStatus == kCVReturnSuccess, let createdPixelBuffer = pixelBuffer {
      CVBufferSetAttachment(createdPixelBuffer,
      ...
      let err = CMVideoFormatDescriptionCreateForImageBuffer(allocator: kCFAllocatorDefault,
  ...
  func render(pixelBuffer: CVPixelBuffer, segmentDatas: UnsafePointer<UInt8>?) -> CVPixelBuffer? {
    ...
    if frameCounter >= frameSkip {
      ...
      extractColorsOptimized(from: inputTexture, with: segmentDatas, width: inputTexture.width, height: inputTexture.height)
    }
    ...
    if segmentationBuffer == nil || segmentationBuffer?.length != bufferSize {
      segmentationBuffer = metalDevice.makeBuffer(bytes: segmentDatas, length: bufferSize, options: .storageModeShared)
    } else {
  ...
  private func extractColorsOptimized(from texture: MTLTexture, with segmentMask: UnsafePointer<UInt8>, width: Int, height: Int) {
    // Create a downsampled version of the texture for color analysis
    guard let downsampledTexture = createDownsampledTexture(from: texture, scale: downsampleFactor) else {
      return
    }
    ...
    guard let textureBuffer = metalDevice.makeBuffer(length: bufferSize, options: .storageModeShared) else {
      return
    }
    ...
    if skinPixelCount > 0 {
      ...
      let newSkinColor = UIColor(red: CGFloat(avgR/255.0), green: CGFloat(avgG/255.0), blue: CGFloat(avgB/255.0), alpha: 1.0)
      
      // Temporal smoothing for stable color values
      if lastColorInfo.skinColor != .clear {
        colorInfo.skinColor = blendColors(newColor: newSkinColor, oldColor: lastColorInfo.skinColor, factor: smoothingFactor)
      } else {
    ...
    if hairPixelCount > 0 {
      ...
      let newHairColor = UIColor(red: CGFloat(avgR/255.0), green: CGFloat(avgG/255.0), blue: CGFloat(avgB/255.0), alpha: 1.0)
      
      // Temporal smoothing for stable color values
      if lastColorInfo.hairColor != .clear {
        colorInfo.hairColor = blendColors(newColor: newHairColor, oldColor: lastColorInfo.hairColor, factor: smoothingFactor)
      } else {
